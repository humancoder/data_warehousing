...
read_csv_auto('./data/citibike-tripdata.csv.gz', header=True, filename=True);

-- Read in csv data
CREATE TABLE central_park_weather as SELECT * FROM
read_csv_auto('./data/central_park_weather.csv', header=True, filename=True);

CREATE TABLE fhv_bases as SELECT * FROM 
read_csv_auto('./data/fhv_bases.csv', header=True, filename=True);
